        ...
        <div style="width: 600px;">
            ...
            <div align="center">
                <%
                    if (screen > 1) {
                        if (request.getParameter("searchid") == null) {%>
                <a href="../JSP/Search_Report.jsp?screen=<%=screen - 1%>&table=<%=request.getParameter("table")%>&sentdataToshow=<%=sentToshow%>">ย้อนกลับ</a>
                <%
                } else if (request.getParameter("searchid") != null) {%>
                <a href="../JSP/Search_Report.jsp?screen=<%=screen - 1%>&table=<%=request.getParameter("table")%>&searchid=<%=new String(request.getParameter("searchid").getBytes("ISO8859-1"), "TIS-620")%>&sentdataToshow=<%=sentToshow%>">ย้อนกลับ</a>
                <%
                        }
                ...
                <%
                    ...
                    if (screen < total_page) {
                        if (request.getParameter("searchid") == null) {%>
                <a href="../JSP/Search_Report.jsp?screen=<%=screen + 1%>&table=<%=request.getParameter("table")%>&sentdataToshow=<%=sentToshow%>">ไปข้างหน้า</a>
                <%
                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") != null) {%>
                <a href="../JSP/Search_Report.jsp?screen=<%=screen + 1%>&table=<%=request.getParameter("table")%>&searchid=<%=new String(request.getParameter("searchid").getBytes("ISO8859-1"), "TIS-620")%>&sentdataToshow=<%=sentToshow%>">ไปข้างหน้า</a>
                <%
                            }
                        } else {
                            ;
