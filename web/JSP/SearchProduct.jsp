    ...
    <form name="serForm" method="get" action="SearchProduct.jsp">
        <div style="width: 600px;">
            <table width="64%" cellpadding="0"  cellspacing="0">
                ...
                <tr >
                    ...
                    <td  colspan="2" class="fcentercenter">
                        <table cellpadding="0" cellspacing="0" >
                            ...
                            <td colspan="2" align="left"><input type="text" name="searchtxt" value="" size="40"/>
                                <a href="javascript:;" ><img src="../IMAGES/BUTTON/MAIN/SEARCH24.gif" width="17" height="17" border="0" onClick="document.serForm.submit()"></a>
                            </td>
                </tr>
                <tr>
                ...
                %>
                <tr>
                    <td colspan="2" align="left"><a  href="#" onClick="gettoShow('<%=getshowdata%>','product_id,name_t_product,pgroup_id,name_t_pgroup,pcat_id,name_t_pcat,ptype_id,name_t_ptype,price,min,max,safety,warehouse_id,name_t_warehouse,location_id,value,remark_product')"><%=rs2.getString("product_id")%></a></td>
                ...
                %>
                        ...
                        if (screen > 1) {
                            if (request.getParameter("searchid") == null && request.getParameter("searchtxt") == null) {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen - 1%>">ย้อนกลับ</a>
            <%
                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") != null) {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen - 1%>&searchid=<%=request.getParameter("searchid")%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>">ย้อนกลับ</a>
            <%
                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") == null) {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen - 1%>&searchid=<%=request.getParameter("searchid")%>">ย้อนกลับ</a>
            <%
                } else {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen - 1%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>">ย้อนกลับ</a>
            <%
                            }
                        }
            ...
            <%
                        ...
                        if (request.getParameter("searchid") == null && request.getParameter("searchtxt") == null) {%>
		| <a href="../JSP/SearchProduct.jsp?screen=<%=i%>"><%=i%></a> |
            <%
                                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") != null) {%>
		| <a href="../JSP/SearchProduct.jsp?screen=<%=i%>&searchid=<%=request.getParameter("searchid")%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>"><%=i%></a> |
            <%
                                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") == null) {%>
		| <a href="../JSP/SearchProduct.jsp?screen=<%=i%>&searchid=<%=request.getParameter("searchid")%>"><%=i%></a> |
            <%
                                } else {%>
		| <a href="../JSP/SearchProduct.jsp?screen=<%=i%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>"><%=i%></a> |
            <%
                        }
                    }
            ...
            <%
                                ...
                                if (request.getParameter("searchid") == null && request.getParameter("searchtxt") == null) {%>
			| <a href="../JSP/SearchProduct.jsp?screen=<%=y%>"><%=y%></a> |
            <%
                                                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") != null) {%>
			| <a href="../JSP/SearchProduct.jsp?screen=<%=y%>&searchid=<%=request.getParameter("searchid")%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>"><%=y%></a> |
            <%
                                                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") == null) {%>
			| <a href="../JSP/SearchProduct.jsp?screen=<%=y%>&searchid=<%=request.getParameter("searchid")%>"><%=y%></a> |
            <%
                                                } else {%>
			| <a href="../JSP/SearchProduct.jsp?screen=<%=y%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>"><%=y%></a> |
            <%
                                    }
                                }
                            }
                        ...
                        if (screen < total_page) {
                            if (request.getParameter("searchid") == null && request.getParameter("searchtxt") == null) {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen + 1%>">ไปข้างหน้า</a>
            <%
                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") != null) {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen + 1%>&searchid=<%=request.getParameter("searchid")%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>">ไปข้างหน้า</a>
            <%
                } else if (request.getParameter("searchid") != null && request.getParameter("searchtxt") == null) {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen + 1%>&searchid=<%=request.getParameter("searchid")%>">ไปข้างหน้า</a>
            <%
                } else {%>
            <a href="../JSP/SearchProduct.jsp?screen=<%=screen + 1%>&searchtxt=<%=new String(request.getParameter("searchtxt").getBytes("ISO8859-1"), "TIS-620")%>">ไปข้างหน้า</a>
            <%
                                }
                            } else {
                                ;
