...
<%
            ...
            switch (statusck) {
                case -1:
                    ...
                    break;
                case 1:
                    /*if (request.getParameter("job_id_end").equals("Y")) {
                        //statusnum = uti.numRowdatabase("Select Count(doc_id) As num from  d_product_receive_header where doc_id = '" + en.EncodeTexttoTIS(request.getParameter("doc_id").trim()) + "' and delete_flag <> 'Y' and complete_flag <> 'Y'");
                        if (uti.numRowdatabase("Select Count(job_id) As num from  d_product_receive_header where job_id = '" + en.EncodeTexttoTIS(request.getParameter("job_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'N'") == 1) {
                            objex.insert(objdb);
                            out.print("บันทึกสำเร็จ");
                        } else {
                            out.print("ไม่สามารถทำการจบใบแจ้งนี้ได้เนื่องจากยังมีใบแจ้งที่ยังค้างอยู่ภายในการผลิต");
                        }
                    ...
                    if (request.getParameter("confirm") != null && request.getParameter("confirm").equals("true")) {
                        if (request.getParameter("job_id_end").equals("Y")) {
                            ...
                            if (uti.numRowdatabase("Select Count(job_id) As num from  d_product_receive_header where job_id = '" + en.EncodeTexttoTIS(request.getParameter("job_id").trim()) + "' and delete_flag = 'N' and complete_flag = 'N'") == 1) {
                                ...
                                out.print("ไม่สามารถทำการจบใบแจ้งนี้ได้เนื่องจากยังมีใบแจ้งที่ยังข้างอยู่ภายในการผลิต");
                            }
