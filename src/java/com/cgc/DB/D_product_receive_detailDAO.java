...
public class D_product_receive_detailDAO {
    ...
    public int insert(DataBeanD_product_receive_detail dataBean) throws Exception {
        ...
        String sql = "insert into d_product_receive_detail "
                +"(doc_id,doc_date,line_no,wh_in,wh_out,burn_no,location_id,size,job_edit_no,ctc_i2,qty,shift,palate_no,emp_id,create_date,wh_id,create_by,unit_id,c_type,job_type_id,job_type_desc,remark,product_id)"
                +" values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(DataBeanD_product_receive_detail dataBean) throws Exception {
        ...
        String sql =
                ...
                + "set  doc_date=?,wh_in=?,wh_out=?,burn_no=?,location_id=?,size=?,job_edit_no=?,ctc_i2=?,qty=?,shift=?,palate_no=?,emp_id=?,update_date=?,wh_id=?,update_by=?,unit_id=?,c_type=?,job_type_id=?,job_type_desc=?,remark=?,product_id=? "
    ...
    public DataBeanD_product_receive_detail ReturnEdit(String Doc_id, String Line_no, DataBeanD_product_receive_detail bean) throws Exception {
        ...
        String SQL = "Select "
                ...
                + "(select munit_code.name_t from munit_code where munit_code.unit_id = d_product_receive_detail.unit_id) as unit_name "
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,doc_date,username','../SCREEN/CS_037.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\">" + rs.getString("line_no") + "</a></td>";
