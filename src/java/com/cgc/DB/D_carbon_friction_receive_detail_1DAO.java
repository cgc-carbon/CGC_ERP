...
public class D_carbon_friction_receive_detail_1DAO {
    ...
    public void insert(DataBeanD_carbon_friction_receive_detail_1 DataBean)throws Exception{
        ...
        try{
            ...
            p.setString(i, DataBean.getSilo_no());i += 1;
            p.setString(i, DataBean.getLocation_id());i += 1;
            p.setString(i, DataBean.getBalance_weight());i += 1;
            p.setString(i, DataBean.getRemain_weight());i += 1;
        ...
        try{
            ...
            if(objuti.numRowdatabase(SQL) == 0){
                ...
                while(rs.next()){
                    ...
                    str_return.append("<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",username,doc_id','../SCREEN/CS_Detail_025_2.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='doc_date' value=\"" + rs.getString("doc_date") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("line_no") + "</a></td>");
