...
public class D_rawmatt_bigbag_withdraw_detailDAO {
    ...
    public int insert(DataBeanD_rawmatt_bigbag_withdraw_detail dataBean) throws Exception {
       //Chk_T_warehouse_item objck = new Chk_T_warehouse_item();
        ...
        String sql = "insert into d_rawmatt_bigbag_withdraw_detail "
                +"(doc_id,line_no,product_id,quantity,weight,cust_id,location_id,create_date,create_by,supplier_id,wh_out,wh_in) "
                +"values(?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(DataBeanD_rawmatt_bigbag_withdraw_detail dataBean) throws Exception {
        ...
        String sql =
                ...
                +"product_id=?,quantity=?,weight=?,cust_id=?,location_id=?,update_date=?,update_by=?,supplier_id=?,wh_out=?,wh_in=? "
        ...
        try {
            /*if (objck.chk_rawmat_item(dataBean.getLocation_id(), dataBean.getProduct_id(), dataBean.getWh_in(), dataBean.getWeight())) {
                ...
                p.setInt(13, Integer.parseInt(dataBean.getRunno()));
                Returnvalue = p.executeUpdate();
           // }
    ...
    public String showDetail(String doc_id) throws Exception {
        ...
        while (rs.next()) {
            ...
            strOutput += "<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,username,runno_" + rs.getString("runno") + "','../SCREEN/CS_058.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("line_no") + "</a></td>";
