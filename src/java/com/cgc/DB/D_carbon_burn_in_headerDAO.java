...
public class D_carbon_burn_in_headerDAO {
    ...
    public int insert(DataBeanD_carbon_burn_in_header dataBean) throws Exception {
        ...
        try {
            ...
            if (objuti.numRowdatabase(sql) != 0) {//ตรวจสอบ Row ใน Detail ว่าหรือไม่
                ...
                sql = "update d_carbon_burn_in_detail set doc_date=?,wh_in=?,wh_out=?,create_by=?,complete_flag=? where doc_id = ? and delete_flag <> 'Y' and complete_flag complete_flag <> 'Y'";
                p = null;
    ...
    private void CopyToD_carbon_burn_in_wh(DataBeanD_carbon_burn_in_header dataBean,Connection con) throws Exception {
        ...
        String SQLinsert_header = "insert into d_carbon_burn_in_header_wh"
                + "(doc_id,doc_date,time_from,time_to,runner_id,shift,burn_no,wh_in,wh_out,total_weight,create_date,update_date,create_by,tank_qty,operate_id,remark,bu,iod,doc_type) "
                + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        String SQLinsert_detail = "insert into d_carbon_burn_in_detail_wh"
                + "(doc_id,doc_date,line_no,product_id,tank_suite,weight,weight_tank,weight_tot,silo_no,location_id,create_date,create_by,channel,remark) "
                + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        ResultSet rs = null;
        PreparedStatement p = null;        
