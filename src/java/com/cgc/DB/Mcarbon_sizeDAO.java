...
public class Mcarbon_sizeDAO {
    ...
    public int insert(DataBeanMcarbon_size dataBean) throws Exception {
        ...
        String sql = "INSERT INTO m_carbon_size(size_id,name_t,remark,create_by,create_date) values(?,?,?,?,?)";
    ...
    public int  update(DataBeanMcarbon_size dataBean) throws Exception {
        DBConnect dbConnect = new DBConnect();
        Connection con = dbConnect.openNewConnection();
        String sql = "UPDATE m_carbon_size SET name_t=?,remark=?,update_by=?,update_date=? where size_id = ? and delete_flag <> 'Y'";
        PreparedStatement p = null;
        ...
        try {
            ...
            p.setInt(5, dataBean.getSize_id());
            returnvalue = p.executeUpdate();
        } finally {
            try {
                p.close();
