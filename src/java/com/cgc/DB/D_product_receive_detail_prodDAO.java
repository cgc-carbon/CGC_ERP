...
public class D_product_receive_detail_prodDAO {
    ...
    public int insert(DataBeanD_product_receive_detail_prod dataBean) throws Exception {
        ...
        String sql = "insert into d_product_receive_detail_prod "
                + "(doc_id,doc_date,line_no,wh_in,wh_out,machine,shift,size,weight,palate_complete_code,palate_no,palate_complete,ctc_i2,bag,emp_id,create_date,create_by,palate_chk,unit_id,job_type_id,job_type_desc,size_by) "
                + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public int update(DataBeanD_product_receive_detail_prod dataBean) throws Exception {
        ...
        String sql = "update d_product_receive_detail_prod set  "
                + "doc_date=?,wh_in=?,wh_out=?,machine=?,shift=?,size=?,weight=?,palate_complete_code=?,palate_no=?,palate_complete=?,ctc_i2=?,bag=?,emp_id=?,update_date=?,update_by=?,palate_chk=?,unit_id=?,job_type_id=?,job_type_desc=?,size_by=? "
    ...
    public DataBeanD_product_receive_detail_prod ReturnEdit(String Doc_id, String Line_no, DataBeanD_product_receive_detail_prod bean) throws Exception {
        ...
        String SQL = "Select doc_id,doc_date,wh_in,wh_out,line_no,machine,shift,size,weight,palate_complete_code,palate_no,palate_complete,ctc_i2,bag,emp_id,"
                + "(select memployee.fname from memployee where memployee.emp_id = d_product_receive_detail_prod.emp_id) as emp_name,palate_chk,unit_id,"
                + "(select munit_code.name_t from munit_code where munit_code.unit_id = d_product_receive_detail_prod.unit_id) as unit_name,job_type_id,job_type_desc,size_by "
	...
	strOutput += "<tr>";
        ...
        strOutput += "<td class='forborder' width='7%'><a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_"+rs.getString("line_no")+",doc_id,doc_date,username','../SCREEN/CS_045.jsp'))\"><input type='hidden' id='line_no_"+rs.getString("line_no")+"' value=\""+rs.getString("line_no")+"\"><input type='hidden' id='doc_id' value=\""+rs.getString("doc_id")+"\">"+rs.getString("line_no")+"</a></td>";
