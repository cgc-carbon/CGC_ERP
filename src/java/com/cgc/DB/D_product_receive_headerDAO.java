...
public class D_product_receive_headerDAO {
    ...
    public int insert(DataBeanD_product_receive_header dataBean) throws Exception {
        ...
        try {
            ...
            if (objuti.numRowdatabase(sql) != 0) {
                sql = "update d_product_receive_detail set wh_in=?,wh_out=?,doc_date=?,create_by=?,c_type=?,job_id=?,complete_flag=? where doc_id = ? and delete_flag = 'N' and complete_flag = 'N'";
                p = null;
                p = con.prepareStatement(sql);
            ...
            if (objuti.numRowdatabase(sql) != 0) {
                sql = "update d_product_receive_detail_prod set wh_in=?,wh_out=?,doc_date=?,create_by=?,c_type=?,job_id=?,complete_flag=?  where doc_id = ? and delete_flag = 'N' and complete_flag = 'N'";
                p = null;
                p = con.prepareStatement(sql);
                p.setString(1, dataBean.getWh_in());
                p.setString(2, dataBean.getWh_out());
    ...
    public int update(DataBeanD_product_receive_header dataBean) throws Exception {
        ...
        try {
            ...
            p.setString(33, dataBean.getDoc_id());
            Returnvalue = p.executeUpdate();
            sql = "select count(doc_id) as num from d_product_receive_detail where doc_id ='" + dataBean.getDoc_id() + "' and delete_flag = 'N' and complete_flag = 'N'";
            if (objuti.numRowdatabase(sql) != 0) {
                sql = "update d_product_receive_detail set wh_in=?,wh_out=?,doc_date=?,create_by=?,job_id=?,complete_flag=? where doc_id = ? and delete_flag = 'N' and complete_flag = 'N'";
                p = null;
                p = con.prepareStatement(sql);
                p.setString(1, dataBean.getWh_in());
                p.setString(2, dataBean.getWh_out());
