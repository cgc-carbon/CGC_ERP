    ...
    public int insert(DataBeanD_carbon_burn_in_detail dataBean) throws Exception {
        ...
        try {
            ...
            if (objuti.numRowdatabase(sql) == 0) {
                sql = "insert into mlocation (location_id,create_by,create_date) "
                        + "values (?,?,?)";
                p = null;
                p = con.prepareStatement(sql);
...
//"+(rs.getString("complete_flag").equals("N")?"<input type='checkbox' id='ckbox' name='ckbox' onclick=\"cancle_chkboxAll('chk_all',this.checked)\">":"&nbsp;")+"
        ...
        try {
            ...
            while (rs.next()) {
                ...
                strOutput.append("<td class='forborder' width='7%'>" + (rs.getString("complete_flag").equals("N") ? "<a href=\"#\" onclick=\"OpenEdit(URLsend('line_no_" + rs.getString("line_no") + ",doc_id,total_weight,username','../SCREEN/CS_028.jsp'))\"><input type='hidden' id='line_no_" + rs.getString("line_no") + "' value=\"" + rs.getString("line_no") + "\"><input type='hidden' id='doc_id' value=\"" + rs.getString("doc_id") + "\"><input type='hidden' id='runno_" + rs.getString("runno") + "'' value=\"" + rs.getString("runno") + "\">" + rs.getString("line_no") + "</a>" : rs.getString("line_no")) + "</td>");
